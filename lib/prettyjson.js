...
exports._addColorToData = function _addColorToData(input) {
	if (options.noColor) {
		return input;
	}
	...
	if (typeof input === 'number') { return pColor.number(sInput); }
...
exports._indentMultiLine = function _indentMultiLine(string, spaces) {
...
exports._handleMultiLine = function _handleMultiLine(data, indentation) {
	return [
...
exports._handleArray = function _handleArray(data, indentation, level) {
...
exports._handleObject = function _handleObject(data, indentation, level) {
	...
	_.each(Object.getOwnPropertyNames(data), function(i) {
		...
		if (Utils.isSerializable(data[i], false, options.inlineArrays) && (!isError || i !== 'stack')) {
			...
			key += Utils.indent(maxIndexLength - i.length) + pColor.depth("[Array] (" + data[i].length + " elements)");
			objOut.push(key);
		} else {
...
exports._render = function _render(data, customOptions, indentation, level) {
...
var maxSortDepth = 20;
var currSortDepth = 0;
exports._sortKeys = function _sortKeys(data, parentIsArray) {
...
exports.render = function(data, customOptions, indentation) {
	...
	pColor = settings.createColorObj(options.colors);
	...
	return exports._render(sortedData, options, indentation);
};


exports.renderString = function renderString(data, customOptions, indentation) {

	var output = '';
	var parsedData;

	options = _.isEmpty(customOptions) ? options : _.defaultsDeep(customOptions, options);
	pColor = settings.createColorObj(options.colors);
