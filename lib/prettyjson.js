...
exports._handleMultiLine = (data, indentation) => {
  return [
    ...
    exports._indentMultiLine(data, indentation + settings.options.defaultIndentation),
    Utils.indent(indentation) + '"""'
  ];
};
...
exports._handleArray = (data, indentation, level) => {
  ...
  var arrayOut = [];
  if (data.length === 0) {
    arrayOut.push(Utils.indent(indentation) + pColor.empty(settings.options.emptyArrayMsg));
  } else if (level + 1 === settings.options.depth) {
    ...
    _.each(data, function (element) {
      ...
      if (Utils.isSerializable(element, false, settings.options.inlineArrays)) {
        // If the element of the array is a string, bool, number, or null
        // render it in the same line
        line += exports._render(element, false);
        arrayOut.push(line);
      } else {
  ...
  _.each(Object.getOwnPropertyNames(data), function (i) {
    ...
    if (Utils.isSerializable(data[i], false, options.inlineArrays) && (!isError || i !== 'stack')) {
      if (_.isFunction(data)) {
        ...
        key += exports._render(data[i], maxIndexLength - i.length);
        objOut.push(key);
      }
...
exports._render = (data, indentation, level) => {
  // Default values
  level = level || 0;
...
exports._sortKeys = (data, parentIsArray) => {
  var sortedData = parentIsArray && !settings.options.numberArrays ? [] : {};
  _.each(_.keys(data).sort(), function (key) {
    if ((_.isObject(data[key]) || _.isArray(data[key])) &&
      currSortDepth < maxSortDepth) {
      currSortDepth++;
      if (_.isArray(data[key])) {
        sortedData[key] = exports._sortKeys(data[key], (!settings.options.numberArrays));
      } else {
...
export const init = (customOptions = {}) => {
  pColor = settings.init(customOptions);
};
...
export const render = (data, startIndent = 0, customOptions = {}) => {
  ...
  var sortedData = data;
  if (settings.options.alphabetizeKeys) {
    sortedData = exports._sortKeys(data, _.isArray(data));
  }
  return exports._render(sortedData, indent);
};
...
export const renderString = (customOptions, data) => {
  ...
  output += exports.render(options, parsedData);
  return output;
};
